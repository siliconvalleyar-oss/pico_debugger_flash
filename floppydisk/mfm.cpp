...
//
// C++ port of Adafruit_Floppy's src/mfm_impl.h.  The flux byte <-> MFM-dibit
// classifier, CRC16-CCITT table and generate/verify logic are kept
// instruction-for-instruction so that disks produced/read here match the
// reference implementation tested on real hardware.
...
typedef struct mfm_io_settings {
    ...
    uint16_t gap_3[8]; /* indexed by 'n' (sector size shift) */
    uint16_t gap_4a;
...
/* gap_3 is indexed by sector-size shift; entry 2 = 512-byte sectors, that is
 * the one used for 1.44M / 720K disks.  Default 108 (FlashFloppy-compatible:
 * makes PC hosts accept 1.44M writes) instead of the PC-spec 84.  The value
 * can be overridden at runtime via mfm_set_gap3_512() (FF.CFG on the SD). */
static mfm_io_settings_t standard_mfm = {
    50, 22, {32, 54, 108, 116, 255, 255, 255, 255}, 80, 12, 0x4e, false};

typedef struct mfm_io {
    uint16_t t1_nom, t2_max, t3_max;
    ...
    uint16_t crc;
    const mfm_io_settings_t *settings;
    void (*flux_byte)(struct mfm_io *, uint8_t);
    void (*encode_raw)(struct mfm_io *, uint8_t);
} mfm_io_t;
...
enum { IMF_IO_IAM = 0xfc, MFM_IO_IDAM = 0xfe, MFM_IO_DAM = 0xfb };
...
static const uint8_t mfm_io_sync_bytes_mfm[] = {0x44, 0x89, 0x44, 0x89, 0x44,
                                                0x89};
static const uint8_t mfm_io_iam_sync_bytes_mfm[] = {0x52, 0x24, 0x52, 0x24,
                                                    0x52, 0x24};
...
static inline bool mfm_io_eof(mfm_io_t *io) { return io->pos >= io->n_pulses; }

static mfm_io_symbol_t mfm_io_read_symbol(mfm_io_t *io) {
    if (mfm_io_eof(io)) return mfm_io_pulse_10;
    uint8_t pulse_len = io->pulses[io->pos++];
    if (pulse_len > io->t3_max) return mfm_io_pulse_1000;
    if (pulse_len > io->t2_max) return mfm_io_pulse_100;
    return mfm_io_pulse_10;
}

/* CRC-16 CCITT, poly 0x11021 */
...
static uint16_t mfm_io_crc16(const uint8_t *data, int len, uint16_t crc) {
    while (len > 0) {
        crc = (uint16_t)(mfm_io_crc16_table[*data ^ (uint8_t)(crc >> 8)] ^
                         (crc << 8));
        data++;
        len--;
    }
...
static bool skip_triple_sync_mark(mfm_io_t *io) {
    ...
    while (!mfm_io_eof(io) && state != mfm_io_triple_mark_magic) {
        state = ((state << 2) | mfm_io_read_symbol(io)) &
                mfm_io_triple_mark_mask;
    }
    return state == mfm_io_triple_mark_magic;
}

/* CRC preload value excluding the three 0xA1 sync bytes */
...
#define PUT_BIT(x)                                                             \
    ...
    /* See mfm_impl.h: we arrive expecting to parse the final '1' of the sync
     * mark; parity bookkeeping recovers the data bit stream from 10/100/1000
     * transition classes. */
    mfm_io_symbol_t s = mfm_io_read_symbol(io);
    mfm_state_t state = mfm_io_even;
    switch (s) {
    case mfm_io_pulse_100:
        state = mfm_io_odd;
        /* fall through */
    case mfm_io_pulse_1000:
        PUT_BIT(0);
        break;
    default:
        break;
    ...
    while ((buf = va_arg(ap, uint8_t *)) != NULL) {
        ...
        while (n) {
            ...
            if (s == mfm_io_pulse_100) {
                if (state) PUT_BIT(0);
                state = (mfm_state_t)!state;
            }

            if (weight <= 0x80) {
                *buf = (uint8_t)(tmp >> 8);
                crc = (uint16_t)(mfm_io_crc16_table[*buf ^ (uint8_t)(crc >> 8)] ^
                                 (crc << 8));
                tmp <<= 8;
                weight <<= 8;
                buf++;
...
static size_t decode_track_mfm(mfm_io_t *io) {
    ...
    for (size_t i = 0; i < io->n_sectors; i++)
        if (io->sector_validity[i]) io->n_valid += 1;
    ...
    while (!mfm_io_eof(io) && io->n_valid < io->n_sectors) {
        if (!skip_triple_sync_mark(io)) continue;

        uint16_t crc = receive_crc(io, &mark, (size_t)1, idam_buf,
                                   (size_t)sizeof(idam_buf), crc_buf,
                                   (size_t)sizeof(crc_buf), (uint8_t *)NULL);

        if (mark != MFM_IO_IDAM) continue;
        if (crc != 0) continue;

        size_t r = (size_t)((uint8_t)idam_buf[2] - 1); /* sectors are 1-based */
        if (r >= io->n_sectors) continue;

        if (io->sector_validity[r]) continue;

        if (!skip_triple_sync_mark(io)) continue;
        size_t io_block_size = 128 << io->n;
        crc = receive_crc(io, &mark, (size_t)1, io->sectors + io_block_size * r,
                          (size_t)io_block_size, crc_buf,
                          (size_t)sizeof(crc_buf), (uint8_t *)NULL);
        if (mark != MFM_IO_DAM) continue;
        if (crc != 0) continue;
...
static void mfm_io_flux_byte(mfm_io_t *io, uint8_t b) {
    for (int i = 8; i-- > 0;) {
        if (b & (1 << i)) {
            ...
            mfm_io_flux_put(io, (uint8_t)((1 + io->pulse_len) * io->t1_nom));
            io->pulse_len = 0;
        } else {
            io->pulse_len += 1;
        }
...
static void mfm_io_encode_raw_mfm(mfm_io_t *io, uint8_t b) {
    uint16_t y = (uint16_t)((io->y << 8) | b);
    if ((b & 0xaa) == 0) {
        /* no clocks present: synthesize them */
        y |= (uint16_t)(~((y >> 1) | (y << 1)) & 0xaaaa);
        y &= 0xff;
    }
    io->flux_byte(io, (uint8_t)y);
    io->y = (uint8_t)y;
}
...
static void mfm_io_encode_fm_sync(mfm_io_t *io, uint8_t data, uint8_t clock) {
    ...
    io->encode_raw(io, (uint8_t)(encoded >> 8));
    io->encode_raw(io, (uint8_t)(encoded & 0xff));
}
...
static void mfm_io_encode_raw_buf(mfm_io_t *io, const uint8_t *buf, size_t n) {
    for (size_t i = 0; i < n; i++) io->encode_raw(io, buf[i]);
}

static void mfm_io_encode_gap(mfm_io_t *io, size_t n_gap) {
    for (size_t i = 0; i < n_gap; i++) mfm_io_encode_byte(io, io->settings->gap_byte);
}

static void mfm_io_encode_gap_and_presync(mfm_io_t *io, size_t n_gap) {
    mfm_io_encode_gap(io, n_gap);
    for (size_t i = 0; i < io->settings->gap_presync; i++) mfm_io_encode_byte(io, 0);
}
...
static void mfm_io_encode_crc(mfm_io_t *io) {
    uint16_t crc = io->crc;
    mfm_io_encode_byte_crc(io, (uint8_t)(crc >> 8));
    mfm_io_encode_byte_crc(io, (uint8_t)(crc & 0xff));
}
...
static size_t encode_track_mfm(mfm_io_t *io) {
    ...
    for (size_t i = 0; i < io->n_sectors; i++) {
        ...
        buf[1] = io->cylinder;
        buf[2] = io->head;
        buf[3] = (unsigned char)(i + 1); /* sectors are 1-based */
        buf[4] = io->n;
        ...
        size_t io_block_size = 128 << io->n;
        mfm_io_encode_buf_crc(io, &io->sectors[io_block_size * i],
                              io_block_size);
        mfm_io_encode_crc(io);
...
void mfm_set_gap3_512(uint16_t gap3) {
    standard_mfm.gap_3[2] = (gap3 >= 60 && gap3 <= 190) ? gap3 : 108;
}

void mfm_timings(mfm_timings_t *t, float nominal_bit_time_us,
                 uint32_t sample_freq_hz) {
    t->t1_nom = (uint16_t)(sample_freq_hz * nominal_bit_time_us * 1e-6f + 0.5f);
    t->t2_max = (uint16_t)(sample_freq_hz * nominal_bit_time_us * 2.5e-6f + 0.5f);
    t->t3_max = (uint16_t)(sample_freq_hz * nominal_bit_time_us * 3.5e-6f + 0.5f);
}
...
size_t mfm_decode_track(const uint8_t *pulses, size_t n_pulses,
    ...
    mfm_io_t io;
    if (clear_validity) memset(validity, 0, n_sectors);
    io.t1_nom = t->t1_nom;
    io.t2_max = t->t2_max;
    io.t3_max = t->t3_max;
